...
def _floor_first_block_candidates(
    ...
    *,
    ...
    if not candidates:
        ...
        # Space Evenly intentionally retains a bounded cross-section sample,
        # which can omit the one-x-plane shapes that Floor First must prefer
        # for a small quantity. Add only those missing plane shapes in that
        # case; larger loads keep the established transverse candidate family.
        transverse_capacity = 0
        for orientation in candidate_orientations:
            _max_nx, max_ny, max_nz = _max_grid_counts(space, orientation)
            transverse_capacity = max(
                transverse_capacity,
                max_ny * max_nz,
            )
        if qty_limit <= transverse_capacity:
            seen = {
                (
                    tuple(round(value, 9) for value in candidate.orientation),
                    candidate.nx,
                    candidate.ny,
                    candidate.nz,
                )
            ...
            for orientation, counts in feasible_shapes:
                nx, ny, nz = counts
                if nx != 1:
                    continue
                signature = (
                ...
                candidates.append(SpaceEvenlyBlockCandidate(
                    ...
                    width=width,
                    height=height,
                    residual_qty=max(0, qty_limit - qty),
                    cross_section_usage=(
                        width * height / max(space.W * space.H, _EPS)
                    ),
                    regularity=min(length, width, height)
...
_FLOOR_FIRST_DEFAULT_RESIDUAL_TRAVERSAL = "row_first_bottom_up"
...
def _floor_first_choose_traversal_placement(
    ...
    These policies are intentionally mode-local. The shared Maximum/Sequence
    placement helper keeps its established best-fit contract. Floor First's
    ordinary default is bottom-up row-first: fill y, then rise in z, then
    advance x. Alternate top/column orders are used only by its bounded
    frontier comparison.
    """
    scores = {
        "row_first_bottom_up": lambda space, waste: (
            space.x,
            space.z,
            space.y,
            waste,
...
def _floor_first_main_block_choice(
    ...
    """Select a Floor First main-block candidate.

    When the requested quantity fits in one transverse ``y × z`` plane, Floor
    First must not advance along ``x`` merely because a longer strip has a
    larger ``W x H`` face. Keep the candidate in one x-plane, fill its width
    row before rising in z, and advance x only after that plane is used. This
    is a geometric rule, so it applies equally to every product and does not
    special-case small-quantity fixtures.
    """
    if qty_limit <= 0:
        return None
    candidates = []
    for space_index, space in enumerate(spaces):
        transverse_capacity = max(
            (
                _max_grid_counts(space, orientation)[1]
                * _max_grid_counts(space, orientation)[2]
            )
            for orientation in group["orientations"]
        )
    ...
    transverse_candidates = [
        ...
        if entry["transverse_capacity"] >= qty_limit
        and entry["candidate"].nx == 1
    ]
    if transverse_candidates:
        candidates = transverse_candidates
        for entry in candidates:
...
def _floor_first_preview_next_block(
    ...
    remaining = min(remaining, _FLOOR_FIRST_FRONTIER_RECTANGLE_LIMIT)
...
def _floor_first_compare_frontier_reflow(
    ...
    The current product's homogeneous interior remains fixed. A bounded suffix
    of its complete continuation (including a zero-, one-, or two-unit loose
    remainder) is reflowed, then the next product's first supported block is
    previewed in the resulting free space. This makes the score describe the
    shared frontier instead of the current product in isolation, without
    branching the rest of the load.
    """
    ...
    # A product frontier is broader than its loose greedy remainder. The
    # homogeneous continuation block is also part of the bounded boundary and
    # must remain eligible when there are zero, one, or two loose residuals.
    # The main block itself remains fixed in this path.
    boundary_limit = min(
        max(_FLOOR_FIRST_FRONTIER_TAIL_WINDOWS),
        len(selected_plan["placements"]),
    )
    fixed_frontier_placements = list(
        selected_plan["placements"][:-boundary_limit]
    )
    frontier_placements = list(
        selected_plan["placements"][-boundary_limit:]
    )
    if not frontier_placements:
        return selected_plan, diagnostics

    reflow_base_spaces = _subtract_cuboids_from_spaces(
        ...
        [
            ...
            for placement in fixed_frontier_placements
        ],
    )
    ...
    priority = 1
    configured_windows = (
        _FLOOR_FIRST_FRONTIER_TAIL_WINDOWS
        if _floor_first_needs_frontier_search(current_metrics)
        else _FLOOR_FIRST_FRONTIER_TAIL_WINDOWS[:1]
    )
    tail_windows = sorted({
        min(tail_qty, len(frontier_placements))
        for tail_qty in configured_windows
    })
    for tail_qty in tail_windows:
        if tail_qty <= 0:
            continue
        tail = frontier_placements[-tail_qty:]
        retained_boundary = frontier_placements[:-tail_qty]
        retained = fixed_frontier_placements + retained_boundary
        retained_cuboids = [
            (
                placement.x,
                placement.y,
        ...
        for orientation in choices:
            for traversal_priority, (policy, _label) in enumerate(
                ...
                retained_block_qty = min(
                    int(selected_plan["continuation_block_qty"]),
                    len(retained),
                )
                plans.append({
                    **selected_plan,
                    ...
                    "continuation_block_qty": retained_block_qty,
                    "residual_greedy_qty": (
                        len(candidate_placements) - retained_block_qty
                    ),
                    "max_x": max(
                        (
                            placement.x + placement.l
    ...
    eligible_plans = [
        plan
        for plan in plans
        if plan["next_preview_qty"] >= plans[0]["next_preview_qty"]
    ]
    selected = max(eligible_plans, key=reflow_score)
    diagnostics.update({
...
def _floor_first_compare_clean_frontier(
    group_start_spaces: List[Space],
    current_spaces: List[Space],
    group: Dict,
    ...
    next_remaining_qty: int,
    next_item_offset: int,
) -> Tuple[List[Space], List[Placement], float, Dict]:
    """Compare a bounded slice at a clean product-to-product boundary.

    Continuation reflow covers products with ordinary leftovers. This helper
    closes the complementary gap: a product whose complete quantity was
    absorbed by its homogeneous main block still has a real boundary with the
    following product. Only the final bounded placement suffix is mutable; the
    homogeneous interior and every earlier product remain fixed.
    """
    diagnostics = {
        "row_index": group["row_index"],
        "next_row_index": (
            next_group["row_index"] if next_group is not None else None
        ),
        "boundary_kind": "clean_main_block",
        "candidates_evaluated": 0,
        "selected_traversal": "floor_first",
        "selected_reflow_orientation": [
            float(value) for value in main_orientation
        ],
        "selected_reflow_tail_qty": 0,
        "traversal_plans": [],
    }
    if (
        not current_placements
        or next_group is None
        or next_remaining_qty <= 0
    ):
        return current_spaces, current_placements, loaded_weight, diagnostics

    boundary_limit = min(
        max(_FLOOR_FIRST_FRONTIER_TAIL_WINDOWS),
        len(current_placements),
    )
    ordered = sorted(
        current_placements,
        key=lambda placement: (
    ...
    fixed_placements = ordered[:-boundary_limit]
    boundary_placements = ordered[-boundary_limit:]
    boundary_base_spaces = _subtract_cuboids_from_spaces(
        [replace(space) for space in group_start_spaces],
        [
            (
                placement.x,
                placement.y,
    ...
    incumbent_metrics = _floor_first_frontier_metrics(
        ordered + incumbent_preview
    )
    plans = [{
        ...
        "loaded_weight": loaded_weight,
        "traversal_policy": "floor_first",
        "reflow_orientation": tuple(main_orientation),
        "reflow_tail_qty": 0,
        "next_preview_qty": (
            int(incumbent_summary["preview_qty"])
            if incumbent_summary is not None
    ...
    priority = 1
    # A clean boundary comes from one homogeneous main block by definition;
    # one bounded suffix window is sufficient to compare every orientation
    # and traversal without multiplying work across equivalent grid slices.
    configured_windows = _FLOOR_FIRST_FRONTIER_TAIL_WINDOWS[:1]
    tail_windows = sorted({
        min(tail_qty, len(boundary_placements))
        for tail_qty in configured_windows
    })
    for tail_qty in tail_windows:
        if tail_qty <= 0:
            continue
        tail = boundary_placements[-tail_qty:]
        retained_boundary = boundary_placements[:-tail_qty]
        branch_spaces = _subtract_cuboids_from_spaces(
            [replace(space) for space in boundary_base_spaces],
            [
                (
                    placement.x,
                    placement.y,
        ...
        branch_loaded_weight = loaded_weight - tail_qty * group["weight"]
        tail_offset = tail[0].item_index - group["item_index_start"]
        for orientation in choices:
            for policy, _label in _FLOOR_FIRST_FRONTIER_TRAVERSALS:
                candidate_spaces = [replace(space) for space in branch_spaces]
                candidate_placements = list(retained_boundary)
                candidate_loaded_weight = branch_loaded_weight
                (
                    candidate_spaces,
                    filled_tail,
                ...
                combined = fixed_placements + candidate_placements + preview
                plans.append({
                    "spaces": candidate_spaces,
                    "placements": fixed_placements + candidate_placements,
                    "loaded_weight": candidate_loaded_weight,
                    "traversal_policy": policy,
                    "reflow_orientation": tuple(orientation),
                    "reflow_tail_qty": tail_qty,
                    ...
                    "frontier_metrics": (
                        _floor_first_frontier_metrics(combined)
                    ),
                    "traversal_priority": priority,
                })
                priority += 1
    ...
    selected = max(eligible_plans, key=reflow_score)
    diagnostics.update({
        "candidates_evaluated": len(plans) - 1,
        "selected_traversal": selected["traversal_policy"],
        "selected_reflow_orientation": [
            float(value) for value in selected["reflow_orientation"]
        ],
        "selected_reflow_tail_qty": selected["reflow_tail_qty"],
        "incumbent_frontier_metrics": incumbent_metrics,
        "selected_frontier_metrics": selected["frontier_metrics"],
        "traversal_plans": [
            {
                "traversal": plan["traversal_policy"],
                "reflow_orientation": [
                    float(value) for value in plan["reflow_orientation"]
                ],
                "reflow_tail_qty": plan["reflow_tail_qty"],
                "next_preview_qty": plan["next_preview_qty"],
                "next_preview_x_end": plan["next_preview_x_end"],
    ...
    return (
        selected["spaces"],
        sorted(selected["placements"], key=lambda placement: placement.item_index),
...
def _floor_first_continuation_candidates(
    ...
    orientation: Tuple[float, float, float],
    *,
    prefer_transverse_plane: bool = True,
) -> List[Dict]:
    ...
    if anchored and not prefer_transverse_plane:
        entries = anchored

    plane_entries = []
    if prefer_transverse_plane:
        for entry in entries:
            space = entry["space"]
            transverse_capacity = int(
                (space.W + _EPS) // orientation[1]
            ) * int((space.H + _EPS) // orientation[2])
            if (
                qty_limit <= transverse_capacity
                and entry["candidate"].nx == 1
            ):
                plane_entries.append(entry)
    if plane_entries:
...
def _floor_first_choose_continuation(
    ...
    *,
    ...
    residual_traversal_policy: str = _FLOOR_FIRST_DEFAULT_RESIDUAL_TRAVERSAL,
    ...
    for orientation_priority, orientation in enumerate(choices):
        candidates = _floor_first_continuation_candidates(
            ...
            main_end_x,
            orientation,
            prefer_transverse_plane=not has_next_frontier,
        )
        ...
        for entry in entries:
            ...
            if residual_qty > 0:
                if not has_next_frontier:
                    branch_spaces, residual_filled, branch_loaded_weight = (
                        _floor_first_fill_row_prefix(
                            branch_spaces,
                            group,
                            residual_qty,
...
def _floor_first_materialize_block(
    ...
    """Materialize a Floor First block with y varying before z, then x."""
    raw = _materialize_grid(
        space,
        candidate.orientation,
        (candidate.nx, candidate.ny, candidate.nz),
...
def _floor_first_fill_group(
    ...
    *,
    ...
    traversal_policy: str = _FLOOR_FIRST_DEFAULT_RESIDUAL_TRAVERSAL,
    ...
    while filled < remaining_qty:
        ...
        if (
            traversal_policy in _FLOOR_FIRST_FRONTIER_TRAVERSAL_POLICIES
            or traversal_policy == _FLOOR_FIRST_DEFAULT_RESIDUAL_TRAVERSAL
        ):
...
def _floor_first_fill_row_prefix(
    spaces: List[Space],
    group: Dict,
    remaining_qty: int,
    item_offset: int,
    ...
    *,
    ...
    """Fill ordinary Floor First residuals as deterministic row prefixes.

    The selected orientation is fixed. Within each selected rectangular free
    space, y varies first, then z, then x. Materializing the supported prefix
    as one batch avoids the staircase residuals produced by item-by-item space
    splitting while leaving frontier reflow semantics unchanged.
    """
    filled = 0
    while filled < remaining_qty:
        payload_units = remaining_qty - filled
        if payload_limit is not None and group["weight"] > 0:
            payload_units = min(
                payload_units,
                max(
        ...
        candidates = []
        for space_index, space in enumerate(spaces):
            if min_x is not None and space.x < min_x - _EPS:
                continue
            max_nx, max_ny, max_nz = _max_grid_counts(space, orientation)
            if not bool(group.get("stackable", True)):
                max_nz = min(max_nz, 1)
            capacity = max_nx * max_ny * max_nz
            if capacity <= 0:
                continue
            candidates.append({
                "space_index": space_index,
                "space": space,
                "counts": (max_nx, max_ny, max_nz),
                "capacity": capacity,
                "score": (
                    space.x,
                    space.z,
                    space.y,
                    -min(capacity, payload_units),
                    -max_ny,
                    -max_nz,
                ),
            })
        ...
        selected = min(candidates, key=lambda entry: entry["score"])
        space = selected["space"]
        max_nx, max_ny, max_nz = selected["counts"]
        batch_qty = min(payload_units, selected["capacity"])
        length, width, height = orientation
        raw = []
        for ix in range(max_nx):
            for iz in range(max_nz):
                for iy in range(max_ny):
                    if len(raw) >= batch_qty:
                        break
                    raw.append((
                        space.x + ix * length,
                        space.y + iy * width,
        ...
        spaces = _subtract_cuboids_from_spaces(spaces, raw)
        loaded_weight += len(raw) * group["weight"]
        filled += len(raw)
...
def _pack_container_floor_first_blocks(
    ...
    for group_index, group in enumerate(groups):
        group_start_spaces = [replace(space) for space in spaces]
        group_placement_start = len(placements)
        next_group = (
        ...
        # Ordinary, non-frontier residuals use the strict row-first order:
        # y, then z, then x. A product that has a following boundary keeps
        # the established local frontier incumbent policy while that
        # boundary is evaluated; it is never propagated to the next product.
        residual_traversal_policy = (
            "floor_first"
            if group_index < len(groups) - 1
            else _FLOOR_FIRST_DEFAULT_RESIDUAL_TRAVERSAL
        )
        ...
        if remaining > 0 and selected is not None:
            ...
            if next_group is not None:
                frontier_evaluations.append({
                    "row_index": group["row_index"],
                    "next_row_index": next_group["row_index"],
                    "boundary_kind": "continuation",
                    "candidates_evaluated": continuation_summary[
                        "traversals_evaluated"
                    ],
                    "selected_traversal": continuation_summary[
                        "selected_traversal"
                    ],
                    "selected_reflow_orientation": continuation_summary[
                        "selected_reflow_orientation"
                    ],
                    "selected_reflow_tail_qty": continuation_summary[
                        "selected_reflow_tail_qty"
                    ],
                    "incumbent_frontier_metrics": continuation_summary[
                        "incumbent_frontier_metrics"
                    ],
                    "selected_frontier_metrics": continuation_summary.get(
                        "selected_frontier_metrics"
                    ),
                    "traversal_plans": continuation_summary[
                        "traversal_plans"
                    ],
                })
        elif remaining > 0:
        ...
        if (
            ...
            current_group_placements = placements[group_placement_start:]
            (
                spaces,
                current_group_placements,
                loaded_weight,
                frontier_summary,
            ) = _floor_first_compare_clean_frontier(
                group_start_spaces,
                spaces,
                group,
                current_group_placements,
                loaded_weight,
                payload_limit,
                tuple(candidate.orientation),
                next_group,
                int(next_group.get("qty", 0) or 0),
                packed_by_row[next_group["row_index"]],
            )
            placements[group_placement_start:] = current_group_placements
            frontier_evaluations.append(frontier_summary)
    ...
    return {
        ...
        "floor_first_frontier_evaluations": frontier_evaluations,
        "floor_first_frontiers_evaluated": sum(
            evaluation["candidates_evaluated"] > 0
        ...
        "floor_first_frontier_traversals_evaluated": sum(
            evaluation["candidates_evaluated"]
            for evaluation in frontier_evaluations
        ),
...
def pack_container(
    ...
    ``maximum_utilization_floor_first``
        ...
        in adjacent side/top spaces with a y-row -> z-layer -> x-strip
