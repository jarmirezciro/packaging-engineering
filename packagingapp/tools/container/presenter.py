...
def selected_container_summary(selected_material, data):
    if selected_material:
        ...
        if weight not in (None, "", "None"):
            meta_parts.insert(0, f"Wt: {weight} g")

        meta = " | ".join([p for p in meta_parts if p])

        return {
            "title": f"{selected_material.part_number} — {selected_material.part_description}",
            "dims": f"{selected_material.part_length} × {selected_material.part_width} × {selected_material.part_height} mm",
            "meta": meta,
        }

    return {
        "title": "Manual container",
        "dims": f'{_read_value(data, "box_l", "")} × {_read_value(data, "box_w", "")} × {_read_value(data, "box_h", "")} mm',
